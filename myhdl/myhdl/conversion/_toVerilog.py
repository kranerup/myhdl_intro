...
from myhdl._ShadowSignal import _TristateSignal, _TristateDriver, ConversionSignal
...
def _checkArgs(arglist):
    for arg in arglist:
        if not isinstance(
            arg,
            (GeneratorType, _Instantiator, _UserVerilogCode, _SignalInstance)
        ):
...
def _makeDoc(doc, indent='', in_ifndef_synthesis=0):
    if doc is None:
        return '', in_ifndef_synthesis
    doc = inspect.cleandoc(doc)
    if _translate_off.match(doc):
        doc = "`ifndef SYNTHESIS"
        in_ifndef_synthesis += 1
        ...
        doc = '// ' + doc
        doc = doc.replace('\n', pre)
    return doc, in_ifndef_synthesis
...
class _ToVerilogConvertor(object):
    ...
    def __call__(self, func, *args, **kwargs):
        ...
        if hasattr(func, "_func"):
            # Ignore @module attribute for top function
            func = func._func
        ...
        if isinstance(func, _Block):
            ...
            try:
                ...
                print(f"func name = {func.__name__}", flush=1)
                raise
            finally:
                _converting = 0
                _converter = None
        ...
        doc, _ = _makeDoc(inspect.getdoc(func))
...
def _writeSigDecls(f, intf, siglist, memlist):
    ...
    for s in siglist:
        ...
        r = _getRangeString(s)
        p = _getSignString(s)
        if isinstance(s, ConversionSignal) and s._direct_assign:
            continue
        if s._driven:
    ...
    for m in memlist:
        ...
        if m._signal_mem:
            for i, s in enumerate(m.mem):
                if s._driven == "reg" and m._driven != "reg":
                    m._driven = "reg"
    ...
    for memport in intf.memports:
        m = intf.memports[memport]
        if m.name not in mems:
            continue
        for i in range(m.depth):
            if m._driven == "reg":
                print(f"always_comb {memport}_{i} = {m.name}[{i}];", file=f)
            elif m._driven == "wire":
                print(f"assign {memport}_{i} = {m.name}[{i}];", file=f)
            else:
                print(f"assign {m.name}[{i}] = {memport}_{i};", file=f)
    print(file=f)
...
def _convertGens(genlist, vfile):
    ...
    for tree in genlist:
        if isinstance(tree, _UserVerilogCode):
            ...
            continue
        elif isinstance(tree, _SignalInstance):
            continue
    ...
    for f in v.funcBuffers:
        vfile.write(f.getvalue())
    funcBuf.close()
...
class _ConvertVisitor(ast.NodeVisitor, _ConversionMixin):
    ...
    def writePreFuncDoc(self, node):
        ...
        if node.doc and _translate.match(node.doc):
            ...
            doc, _ = _makeDoc(node.doc, self.ind)
            self.write(doc)
        self.writeline()
    ...
    def writeAlwaysHeader(self):
        ...
        if toVerilog.standard == "systemverilog":
            if clocked:
                ...
                always = "always_comb"
                has_senslist = False
        vattr = getattr(self.tree.always, "_verilog_attr", {})
        if pf := vattr.get("prefix"):
            self.write(pf)
            self.writeline()
        self.write(f"{always} ")
        if has_senslist:
    ...
    def writeAlwaysFooter(self):
        self.dedent()
        self.writeline()
        self.write("end")
        vattr = getattr(self.tree.always, "_verilog_attr", {})
        if sf := vattr.get("suffix"):
            self.writeline()
            self.write(sf)
            self.writeline()
        self.writeline(2)
    ...
    def visit_Assert(self, node):
        ...
        if not self.in_ifndef_synthesis:
            self.write("`ifndef SYNTHESIS")
            self.writeline()
        if not assert_false:
            self.write("if (")
            ...
            self.indent()
        self.writeline()
        self.write('$display("*** AssertionError *** in %m at %t ", $time);')
        self.writeline()
        if node.msg:
            self.visit_Print(node, print_guard=False)
            self.writeline()
        self.write('$finish;')
        if not assert_false:
            self.dedent()
...
#            self.writeline()
    ...
    def visit_Call(self, node):
        ...
        if hasattr(node, 'tree'):
            ...
            self.funcStack.append( self.funcBuf )
            self.funcBuf = StringIO()

            v = Visitor(node.tree, self.funcBuf)
            v.funcStack = self.funcStack
            v.funcBuffers = self.funcBuffers
            v.visit(node.tree)

            self.funcBuffers.append( self.funcBuf )
            self.funcBuf = self.funcStack.pop()
    ...
    def visit_Expr(self, node):
        ...
        if isinstance(expr, ast.Str):
            doc, self.in_ifndef_synthesis = _makeDoc(
                expr.s, self.ind, self.in_ifndef_synthesis
            )
            self.write(doc)
            return
...
#        self.writeline()
    ...
    def visit_Print(self, node, print_guard=True):
        argnr = 0
        if print_guard:
            if not self.in_ifndef_synthesis:
                self.write("`ifndef SYNTHESIS")
                self.writeline()
            self.write("`ifndef NO_PRINTS")
            self.writeline()
    ...
    def visit_Yield(self, node):
        ...
        # We support an empty senslist so that yield None
        # works (produces no verilog code). Possibly breaks
        # some case that I'm not aware of.
...
def _annotateTypes(genlist):
    for tree in genlist:
        if isinstance(tree, (_UserVerilogCode, _SignalInstance)):
            continue
