...
from myhdl._ShadowSignal import (
    _ShadowSignal, _SliceSignal, _TristateDriver, ConversionSignal,
)
    ...
    for inst in hierarchy:
        ...
        for n, s in sigdict.items():
            if s._name is not None:
                continue
            if isinstance(s, ConversionSignal):
                pass
            elif isinstance(s, _SliceSignal):
                continue
...
def _analyzeGens(top, absnames):
    ...
    for g in top:
        if isinstance(g, _UserCode):
            ...
            v.visit(tree)
        elif isinstance(g, _SignalInstance):
            continue
        else:  # @instance
            f = g.gen.gi_frame
            tree = g.ast
...
# #         arg.target = self.getObj(node.expr)
    ...
    def visit_Yield(self, node, *args):
        ...
        if isinstance(n, ast.Tuple):
            ...
            print("ERROR n is",n,n.obj,n.value,flush=1)
            self.raiseError(node, _error.UnsupportedYield)
...
def expandinterface(v, name, obj):
    for attr, attrobj in vars(obj).items():
        if isinstance(attrobj, _Signal):
            signame = f"{name}_{attr}"
            attrobj._name = signame
            v.argdict[signame] = attrobj
        elif isinstance(attrobj, myhdl.EnumType):
            pass
        elif isinstance(attrobj, Struct):
            # can assume is yet another interface ...
            signame = f"{name}_{attr}"
            expandinterface(v, signame, attrobj)
        elif isinstance(attrobj, list):
            signame = f"{name}_{attr}"
            expandlist(v, signame, attrobj)


def expandlist(v, name, obj):
    try:
        meminfo = _getMemInfo(obj)
        v.memports[name] = meminfo
    except KeyError:
        meminfo = None
    for i, x in enumerate(obj):
        signame = f"{name}_{i}"
        if isinstance(x, list):
            expandlist(v, signame, x)
        elif isinstance(x, _Signal):
            v.argdict[signame] = x
            if meminfo:
                x._driven = meminfo._driven or x._driven
                x._read = meminfo._read or x._read
...
class _AnalyzeTopFuncVisitor(_AnalyzeVisitor):
    ...
    def _arg(self, name, arg):
        if isinstance(arg, _Signal):
            self.argdict[name] = arg
            arg._name = name
        elif isinstance(arg, Struct):
            expandinterface(self, name, arg)
        elif isinstance(arg, list):
            expandlist(self, name, arg)
    ...
    def visit_FunctionDef(self, node):
        ...
        for n in self.argnames[i + 1:]:
            if n in self.kwargs:
                ...
                self.fullargdict[n] = arg
                self._arg(n, arg)
        self.argnames = list(self.argdict.keys())
